...
This file is read by Codex on every session in this repo. Treat the rules below as non-negotiable.
...
Current phase deliverables: [`plan/phase-3-deliverables.md`](./plan/phase-3-deliverables.md)
...
7. **Update AGENTS.md and README.md at every milestone PR** (currently at the end of each Phase). Stale docs are worse than no docs.
...
├── .Codex/                   ← Codex project settings + skills
│   ├── settings.json
│   └── skills/
├── AGENTS.md                  ← this file
├── README.md
...
- Always update the active phase deliverables file status column when a PR merges

## Current state

- **Active phase:** Phase 3 — Hiring Workflow Depth
- **Active deliverable:** PR-21 · Phase 3 kickoff planning on `qa001-phase3-plan`
- **Last merged:** PR-20 · QuickBooks export baseline + Stripe fallback invoice handoff scaffold
- **Next focus:** PR-22 · Submission domain baseline

### What is live in dev now

- Auth0 SPA login/logout with `/me` verification and tenant-aware claims
- Tenant-scoped EF query filtering and request tenant resolution
- Marketing pages (`/`, `/about`, `/services`, `/contact`)
- Public jobs list/detail plus guest apply flow
- Candidate dashboard with profile save, resume metadata support, and applied jobs history
- Contractor dashboard with weekly draft/save/submit timesheet flow
- Client dashboard with submitted-week approve/reject flow
- Recruiter workspace with job creation, application stage movement, invoice staging, and billing handoff previews
- Demo startup seeding for two tenants in the shared dev environment
- SPA fallback routing plus explicit 404 handling
